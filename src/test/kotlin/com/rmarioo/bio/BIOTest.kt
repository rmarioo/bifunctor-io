...
class BIOTest {
    ...
    fun referentialTransparency() {
        ...
        assertThat(p1Refactored).isNotEqualTo(p1WithDuplication)

        val p2WithDuplication               = executeProgram { `p2 with duplication`()}
        val p2Refactored                    = executeProgram { `p2 refactored`() }

        assertThat(p2Refactored).isEqualTo(p2WithDuplication)
        assertThat(p2Refactored).isEqualTo(p1WithDuplication)
    }
